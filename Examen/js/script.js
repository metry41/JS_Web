...
if( !validaCorreo() || !validaClave() || !validaNombre() || !validaFecha() )
	...
	return true;
//alert("FunciÃ³n validaDatos concluida");
}
...
//window.document.forms[0].Correo.value.indexOf('@') serÃƒÂ­a el formulario 0, el primero y ÃƒÂºnico que tenemos
...
con este with nos ahorramos poner la ruta completa de los elementos
  de nuestro fomulario encuesta  habrÃƒÂ­a que poner todo a lo que abarque entre llaves {}
...
if(esBisiesto(aa))
	array_meses[1]=29;
// Lo anterior deberÃƒÂ­a ir con el if final para comprobar el dÃƒÂ­a 
// y la funciÃƒÂ³n bisiesto solo deberÃƒÂ­a llamarla si el mes es 2 if(mm==2 && bisiesto(aa))
if (!esAnio(aa))
	return false;
else
	if (!esMes(mm))
...
/*************************************************************
Funciones "externas" invocadas desde el formulario
en procesos automÃ¡ticos relacionados con el formato de los campos
o el almacenamiento de los datos

* ***********************************************************/

//cambia el correo electrÃ³nico todo a minÃºsculas
...
//DeberÃƒÂ­a coger igual comillas dobles que simples para cadenas de caracteres
...
function fechaActual(sLugar) {
	...
	var dFecha = new Date();
	//Para obtener el aÃƒÂ±o utilizamos el mÃƒÂ©todo getYear():
	var anio = dFecha.getYear() + 1900;
	//getDay() devuelve 0 para domingo y asÃƒÂ­ sucesivamente
	var diaSem = dFecha.getDay();
	//getDate() devuleve el dÃƒÂ­a del mes
	var diaMes = dFecha.getDate();
	//getMonth() devuelve 0 para enero
	var mes = dFecha.getMonth();
	return ('En ' + sLugar + ', el ' + aDias[diaSem] + ' dÃ­a ' + diaMes + ' de ' + aMeses[mes] + ' del aÃ±o ' + anio);
}
...
with (window.document.forms.encuesta)
	...
	for (i=0;i<length;i++)
		...
		if (elements[i].type!=undefined)
			...
			//alert("Elemento nÃ‚Âº "+i+' de tipo type='+elements[i].type);
			}		
		}
	}
}
...
if (Ele1.value.length > 0 )
	...
	Ele2.disabled=false;
	Ele2.focus();
//El metodo focus lo que hace es llevar el cursor a este elemento, a execepciÃƒÂ³n del intrÃƒÂ­nseco que es Ele1
	}
else
	{
	Ele2.disabled=true;
	...
	with (window.document.forms.encuesta)
	if (Ele2==Apellido1)
	//hemos preguntado por el atributo o propiedad name del objeto, a ver si es Apellido1, lo coge asÃƒÂ­ por sintaxis
		{
		Apellido2.value='';
